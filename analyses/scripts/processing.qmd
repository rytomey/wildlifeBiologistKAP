...
psurv <- surv %>% dplyr::select(ID, RESIDENCE, noSOURCE)
psurv
...
psurv$AFFILIATEbin <- ifelse(surv$AFFILIATE %in% c("Federal Government", "State/Provincial Government"), 1,
                        ifelse(is.na(surv$AFFILIATE), NA, 0))
table(psurv$AFFILIATEbinbin)
...
psurv$ACTIVITYord <- factor(surv$ACTIVITY, ordered = TRUE,
              levels = c("Land/Wildlife Management", "Research", "Administration", 
                                       "Consulting", "Education", "Other"))
table(psurv$ACTIVITYord)
...
psurv$CCORRECTmed <- ifelse(surv$CCORRECT >= med_ccorrect, 1, 0)
...
psurv$CCORRECTavg <- ifelse(surv$CCORRECT >= med_ccorrect, 1, 0)
...
attitude_ord <- paste0(attitude_items, "ord")

# Step 2: Create indicator matrices for each response type
psurv$AGREEnum    <- rowSums(sapply(attitude_ord, function(x) psurv[[x]] == "agree"), na.rm = TRUE)
psurv$NEUTRALnum  <- rowSums(sapply(attitude_ord, function(x) psurv[[x]] == "neutral"), na.rm = TRUE)
psurv$DISAGREEnum <- rowSums(sapply(attitude_ord, function(x) psurv[[x]] == "disagree"), na.rm = TRUE)

# Optional: Check the distribution of each
table(psurv$AGREEnum)
table(psurv$NEUTRALnum)
table(psurv$DISAGREEnum)
...
writexl::write_xlsx(summary_tables, path = file.path(oup, "summaryDFS.xlsx"))
